...
//            PORT_PICAM_MARKUP = piCamPorts[1],
            PORT_LIFECAM = lifecamPorts[0],
            PORT_LIFECAM2 = lifecamPorts[1];
        ...
        MjpegServer
            lifecamServer = new MjpegServer("LIFECAM_RAW", PORT_LIFECAM),
            lifecam2Server = new MjpegServer("LIFECAM_RAW2", PORT_LIFECAM2);
...
//            picamMarkupServer = new MjpegServer("PICAM_MARKUP", PORT_PICAM_MARKUP);
        ...
        CvSource lifecamSource = new CvSource("LIFECAM_COMPRESSED", VideoMode.PixelFormat.kMJPEG, RES_X, RES_Y, FPS);
        CvSource lifecam2Source = new CvSource("LIFECAM_COMPRESSED", VideoMode.PixelFormat.kMJPEG, RES_X, RES_Y, FPS);
        ...
        UsbCamera lifecam2 = new UsbCamera("Lifecam2_3000", CscoreSettings.getLifecam2ID());
        

        // Our CvSink
        CvSink lifecamSink = new CvSink("CvSink_Lifecam");
        CvSink lifecam2Sink = new CvSink("CvSink_Lifecam2");
        ...
        lifecamServer.setSource(lifecam);
        lifecamSink.setSource(lifecam);
        lifecam2Server.setSource(lifecam2);
        lifecam2Sink.setSource(lifecam2);

        // Add threads to thread pool
        THREAD_POOL.execute(new VisionTask(lifecamSource, lifecamSink, new BasicFilterPipeline(), rootTable));
        THREAD_POOL.execute(new VisionTask(lifecam2Source, lifecam2Sink, new BasicFilterPipeline(), rootTable));
